...
const FootballCard = ({ match, onClick, onAuthRequired }) => {
    const { isMatchFollowed, toggleFollowMatch } = useFollow();
    const { isAuthenticated } = useAuth();
    const isFollowed = isMatchFollowed(match.id, 'football');
    ...
    const isCompleted = statusLower.includes('finished') ||
        ...
        statusLower.includes('match finished');
    ...
    const handleFollowClick = (e) => {
        ...
        toggleFollowMatch(match.id, 'football');
    };
        ...
        >
            ...
            <div className="flex items-start justify-between mb-4">
                <div className="flex-1 min-w-0">
                    {/* League Badge */}
                    <div className="mb-2">
                        <span className="px-2.5 py-1 text-xs font-semibold rounded-full bg-cyan-100 text-cyan-700 border border-cyan-200">
            ...
            <div className="space-y-3 flex-1 flex flex-col justify-center">
                {/* Team 1 */}
                <div className="flex items-center justify-between p-3 bg-gradient-to-r from-cyan-50 to-blue-50 rounded-xl border border-cyan-200 hover:border-cyan-300 transition-colors">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                        <p className="font-bold text-gray-900 text-base truncate">{team1}</p>
                    </div>
                    {score1 !== null && score1 !== undefined && (
                        <span className="text-2xl font-extrabold text-cyan-700 ml-3 whitespace-nowrap">
                            {score1}
                        </span>
                    )}
                    {score1 === null && (
