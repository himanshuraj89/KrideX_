...
const CricketCard = ({ match, onClick, onAuthRequired }) => {
    const { isMatchFollowed, toggleFollowMatch } = useFollow();
    const { isAuthenticated } = useAuth();
    const isFollowed = isMatchFollowed(match.id, 'cricket');
    ...
    const isLive = statusLower.includes('live') ||
        ...
        statusLower.includes('elected to') ||
        statusLower.includes('chosen to') ||
    ...
    const formatDate = (dateString, dateTimeGMT) => {
        // Use dateTimeGMT if available as it has precise time
        const dateToUse = dateTimeGMT ? new Date(dateTimeGMT) : (dateString ? new Date(dateString) : null);
    ...
    const getScore = () => {
        ...
        const score1 = match.score[0]?.r ?? match.score[0]?.score ?? null;
        const score2 = match.score[1]?.r ?? match.score[1]?.score ?? null;
    ...
    const handleFollowClick = (e) => {
        ...
        toggleFollowMatch(match.id, 'cricket');
    };
    ...
    return (
        ...
        >
                    ...
                    {match.date && (
                        <div className="mb-2 flex items-center gap-1.5">
                            ...
                            <span className="text-xs text-gray-500">
                                {formatDate(match.date, match.dateTimeGMT)}
                            </span>
                        </div>
                    )}
                    {/* Venue */}
            ...
            <div className="space-y-3 flex-1 flex flex-col justify-center">
                {/* Team 1 */}
                <div className="flex items-center justify-between p-3 bg-gradient-to-r from-emerald-50 to-green-50 rounded-xl border border-emerald-200 hover:border-emerald-300 transition-colors">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                        <p className="font-bold text-gray-900 text-base truncate">{team1}</p>
                    </div>
                    {score1 !== null && score1 !== undefined && (
                        <span className="text-2xl font-extrabold text-emerald-700 ml-3 whitespace-nowrap">
                            {score1}
                        </span>
                    )}
                    {score1 === null && (
