...
const BasketballCard = ({ match, onClick, onAuthRequired }) => {
    const { isMatchFollowed, toggleFollowMatch } = useFollow();
    const { isAuthenticated } = useAuth();
    const isFollowed = isMatchFollowed(match.id, 'basketball');
    ...
    const getScore = () => {
        ...
        const score1 = match.score[0]?.points ?? match.score[0]?.score ?? null;
        const score2 = match.score[1]?.points ?? match.score[1]?.score ?? null;
    ...
    const handleFollowClick = (e) => {
        ...
        toggleFollowMatch(match.id, 'basketball');
    };
        ...
        >
            ...
            <div className="flex items-start justify-between mb-4">
                <div className="flex-1 min-w-0">
                    {/* League Badge */}
                    <div className="mb-2">
                        <span className="px-2.5 py-1 text-xs font-semibold rounded-full bg-orange-100 text-orange-700 border border-orange-200">
                            {match.league || 'NBA'}
                        </span>
                    </div>
            ...
            <div className="space-y-3 flex-1 flex flex-col justify-center">
                {/* Team 1 */}
                <div className="flex items-center justify-between p-3 bg-gradient-to-r from-orange-50 to-red-50 rounded-xl border border-orange-200 hover:border-orange-300 transition-colors">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                        <p className="font-bold text-gray-900 text-base truncate">{team1}</p>
                    </div>
                    {score1 !== null && score1 !== undefined && (
                        <span className="text-2xl font-extrabold text-orange-700 ml-3 whitespace-nowrap">
                            {score1}
                        </span>
                    )}
                    {score1 === null && (
